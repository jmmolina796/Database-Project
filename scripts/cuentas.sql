-- Tables: cuentas, cat_bancos, clientes
...
BEGIN
	...
	FOR i IN 1..num LOOP
		...
		id_banco_c := my.id_banco FROM ( SELECT row_number() over() AS numb, cb.id_banco FROM cat_bancos cb ) AS my WHERE numb = i;
		id_banco_l := LEFT(my.id_banco::VARCHAR,3) FROM ( SELECT row_number() over() AS numb, cb.id_banco FROM cat_bancos cb ) AS my WHERE numb = i;
